package it.unina.cinemates.viewmodels.user.profile.other;
...
public class ProfileViewModel extends ViewModel {
    ...
    private static final String TAG = "OTHERUSERPROFILE_VIEWMODEL";
    ...
    public void fetchUserProfile(String username) {
        PostDataInMutableLiveDataCallback<UserProfile> callback = new PostDataInMutableLiveDataCallback<UserProfile>(this.userProfile, TAG) {
            ...
            public void onResponse(@NonNull Call<UserProfile> call, Response<UserProfile> response) {
                ...
                if (response.code() == 200) {
                    ...
                    if(response.body().getMovieLists() != null) {
        ...
        BackendRetrofitService.getInstance().getUserAPI().getUser(username, LoggedUser.getInstance().getUsername()).enqueue(callback);
    }
